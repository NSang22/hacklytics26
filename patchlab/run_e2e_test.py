...
def main():
    print("=" * 60)
    print("  PatchLab — Full E2E Test (Mario 1-1)")
    print("=" * 60)
    ...
    proj_body = {
        ...
        "dfa_states": [
            {"name": "Overworld_Start", "description": "Opening run — flat ground, first Goomba, ? blocks",
             "intended_emotion": "excited", "acceptable_range": [0.3, 0.8],
             "expected_duration_sec": 8,
             "visual_cues": ["flat ground", "goomba", "question blocks"],
             "failure_indicators": ["repeated deaths to first goomba", "frozen in place"],
             "success_indicators": ["stomps goomba", "hits ? block"]},
            {"name": "Block_Discovery", "description": "Player discovers ? blocks give power-ups",
             "intended_emotion": "curious", "acceptable_range": [0.3, 0.7],
             "expected_duration_sec": 5,
             "visual_cues": ["? blocks", "mushroom emerging", "coin particles"],
             "failure_indicators": ["runs past all blocks"],
             "success_indicators": ["hits blocks", "grabs mushroom"]},
            {"name": "Platforming", "description": "Pipes, pits, enemies. Deaths return to start; only excessive deaths are concerning.",
             "intended_emotion": "tense", "acceptable_range": [0.3, 0.7],
            ...
            {"name": "Mid_Gauntlet", "description": "Dense enemy section — deaths respawn to start, only death loops matter",
             ...
             "visual_cues": ["multiple enemies", "elevated platforms", "koopa shells"],
             "failure_indicators": ["dies 3+ times consecutively", "stops moving"],
             "success_indicators": ["chain stomps", "maintains flow"]},
            {"name": "Flagpole", "description": "Staircase and flagpole — victory moment",
             "intended_emotion": "delight", "acceptable_range": [0.5, 1.0],
             "expected_duration_sec": 6,
             "visual_cues": ["staircase", "flagpole", "castle"],
             "failure_indicators": ["falls off stairs repeatedly"],
        ...
        "transitions": [
            ...
            {"from_state": "Mid_Gauntlet", "to_state": "Overworld_Start", "trigger": "player dies"},
        ],
    }
