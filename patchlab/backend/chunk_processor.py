...
def _build_gemini_prompt(
    ...
    Instead of asking Gemini to classify a video holistically, this prompt
    ...
    # Build transition graph or fall back to linear progression
    if dfa_config.transitions:
        transition_desc = "\nVALID TRANSITIONS:\n" + "\n".join(
            f"  {t.from_state} -> {t.to_state} (trigger: {t.trigger})"
...
async def process_chunk(
    ...
    if not frames:
        logger.warning(f"[chunk_processor] Frame extraction yielded 0 frames for chunk {chunk_index} — falling back to mock")
        return _generate_mock_result(chunk_index, chunk_start_sec, dfa_config, previous_context)
    if emotion_frames:
